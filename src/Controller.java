...
public class Controller {
	...
	public static void main(String[] args) {
		...
		if (jeu == 0) {
			...
			// Ainsi on a pose ce double au milieu et ajoute les extremites au tableau d'extremite
	...
	// public static void ajouterListnerBoutonTriomino(JButton
	// boutton,ArrayList<PieceTriomino> deck,JoueurTriomino joueur,int
	// nbJoueur,TableTriomino tableTriomino){
	//
	// boutton.addMouseListener(new MouseAdapter() {
	// @Override
	// public void mouseClicked(MouseEvent e) {
	// if (!partieCommance) {
	// joueur.piocher(deck, 1);
	// piocheOk = true;
	// } else if (boutton.isEnabled()) {
	// joueur.piocher(deck, 1);
	// joueur.setScore(joueur.getScore() - 5);
	// joueur.setNombreDePioches(
	// joueur.getNombreDePioches() + 1);
	// if (joueur.getNombreDePioches() >= 3) {
	// boutton.setEnabled(false);
	// joueur
	// .setScore(joueur.getScore() - 10);
	// }
	// tableTriomino.dessinerPiecesJoueur(nbJoueur, joueur.getMain());
	// tableTriomino.setScoreJoueur(nbJoueur, joueur.getScore());
	// }
	//
	// }
	// });
